...
function containerPort(dolphin, containerId, opts) {
  return dolphin.containers.inspect(containerId).then(function (container) {
    opts = opts || {};
    var port = opts.port || Object.keys(container.NetworkSettings.Ports)[0].split('/')[0];

    var netNames = Object.keys(container.NetworkSettings.Networks);
    var network;
    if (netNames.length === 1) {
      network = container.NetworkSettings.Networks[netNames[0]];
    } else if (opts.networkName && netNames.indexOf(opts.networkName) > -1) {
      network = container.NetworkSettings.Networks[opts.networkName];
    } else {
